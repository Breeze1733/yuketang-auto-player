...
本脚本**专注于课程视频与课件的自动播放**。已彻底移除所有 AI 答题与 OCR 代码，遇到所有作业、测验与考试节点均会自动跳过，回归纯粹、安全、轻量的课程学习播放器辅助。
...
### 1. 纯净视频播放定位（已剔除 AI 答题）
* **自动跳过作业与习题**：脚本在执行过程中，检测到任何作业、课后测验、考试节点时，均会**直接跳过，不作答任何题目**。
* **零外部 CDN 依赖**：彻底剥离了 `tesseract.js`、`html2canvas` 等庞大的第三方 OCR 库和所有外部大模型 API 请求，代码体积缩减 30%，加载秒开，运行更稳定轻巧。
