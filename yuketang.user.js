...
// @grant        unsafeWindow
// @homepageURL  https://github.com/Breeze1733/yuketang-auto-player
// @supportURL   https://github.com/Breeze1733/yuketang-auto-player/issues
// @updateURL    https://raw.githubusercontent.com/Breeze1733/yuketang-auto-player/main/yuketang.user.js
// @downloadURL  https://raw.githubusercontent.com/Breeze1733/yuketang-auto-player/main/yuketang.user.js
// ==/UserScript==
...
(() => {
  ...
  const Store = {
    getPlaybackConf() {
      const raw = localStorage.getItem(Config.storageKeys.playback);
      const saved = Utils.safeJSONParse(raw, {}) || {};
      const conf = {
        playbackRate: Number(saved.playbackRate) || Config.playbackRate,
        pptInterval: Number(saved.pptInterval) || Config.pptInterval,
        autoComment: Boolean(saved.autoComment)
      };
      return conf;
    },
    setPlaybackConf(conf) {
      localStorage.setItem(Config.storageKeys.playback, JSON.stringify(conf));
      if (conf.playbackRate) Config.playbackRate = conf.playbackRate;
      if (conf.pptInterval) Config.pptInterval = conf.pptInterval;
    },
    getProgress(url) {
  ...
  function createPanel() {
            ...
            <div class="panel" id="panel">
              ...
              <div class="body">
                <ul class="info" id="info">
                  ...
                  <li>🚀 点击下方<span style="color:#1677ff">[开始刷课]</span>即可开始自动播放</li>
              ...
              <div id="settings">
                <div class="form-item">
                  <label>视频播放倍速：</label>
                  <select id="setting_rate" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:12px;">
                    <option value="1">1.0x 正常速度</option>
                    <option value="1.25">1.25x 倍速</option>
                    <option value="1.5">1.5x 倍速</option>
                    <option value="2" selected>2.0x 倍速（推荐）</option>
                  </select>
                </div>
                <div class="form-item">
                  <label>PPT 翻页间隔时间 (毫秒)：</label>
                  <input type="number" id="setting_ppt_interval" value="3000" min="1000" step="500" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:12px;box-sizing:border-box;">
                </div>
                <div class="form-item">
              ...
              <div class="footer">
                <button id="btn-setting">播放设置</button>
    ...
    const ui = {
      ...
      settingRate: doc.getElementById('setting_rate'),
      settingPptInterval: doc.getElementById('setting_ppt_interval'),
    ...
    const loadPlaybackSettings = () => {
      const conf = Store.getPlaybackConf();
      ui.settingRate.value = String(conf.playbackRate);
      ui.settingPptInterval.value = String(conf.pptInterval);
      ui.featureAutoComment.checked = conf.autoComment;
    };
    ...
    ui.saveSettings.onclick = () => {
      const conf = {
        playbackRate: Number(ui.settingRate.value) || 2,
        pptInterval: Number(ui.settingPptInterval.value) || 3000,
        autoComment: ui.featureAutoComment.checked
      };
      Store.setPlaybackConf(conf);
      ui.settings.style.display = 'none';
      log(`✅ 播放设置已更新：倍速 ${conf.playbackRate}x，PPT间隔 ${conf.pptInterval}ms`);
    };
  ...
  class V2Runner {
    ...
    // 作业节点：定位为纯视频播放，自动跳过答题
    async handleHomework(item, idx) {
      const title = item.querySelector('h2')?.innerText || `第${idx + 1}项`;
      this.panel.log(`检测到作业/测验：${title}（定位为纯视频播放，自动跳过答题）`);
      idx++;
      this.updateProgress(this.outside, idx);
      return idx;
    }
    ...
    async run() {
      ...
      while (true) {
        ...
        if (classType.includes('tuwen') && !classStatus.includes('已读')) {
          ...
          this.panel.log(`进入作业：${className}（定位为纯视频播放，已自动跳过答题）`);
          await Utils.sleep(2000);
        } else if (classType.includes('kaoshi')) {
          this.panel.log(`进入考试：${className}（定位为纯视频播放，已自动跳过）`);
          await Utils.sleep(2000);
  ...
  class AiWorkspaceRunner {
    ...
    async handleExercise(route) {
      const title = AiWorkspace.getActiveLeafTitle() || route.leafId;
      this.panel.log(`检测到作业/习题（${title}），定位为纯视频播放，自动跳过答题`);
      await Utils.sleep(1200);
      return true;
    }
